...
#  "selection":{"metric":"roc_auc","split":"test","score":0.8165}}
...
curl 'localhost:8000/datasets/inference_predictions?limit=2&offset=0'
curl 'localhost:8000/datasets/raw_modelling_data?limit=5'
```
...
Unknown or misspelled fields are rejected with **422** (`extra="forbid"`) rather
than silently ignored, and out-of-range values are rejected the same way.
...
**`/ready` returns 503, or `DatasetError: Failed while loading data from production_model`**
The model has not been trained yet. Run `uv run kedro run --pipeline training`.
The API retries the load on the next request — no restart needed.
